...
dim(validation)
```
The results below show the top 10 geneneres
```{r}
...
The 1st model predicts rating using the dataset's mean rating, and all differences in movie ratings are explained by random variation. Following is the equation used for the calculation: Yu,i = ?? + ??u,i
...
It is found that there is substantial variability across users: some users rate many movies while others are selective. Hence, the average rating for user ?? is only computed for those that have rated over 100 movies, and the following equation is used for the prediction: Yu,i = ?? + bi + bu + ??u,i
